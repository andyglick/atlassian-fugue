...
import com.atlassian.fugue.optic.POptional;
...
public final class OptionalLaws<S, A> {
  ...
  public IsEq<List<S>> modifyStreamFPoint(S s) {
    return isEq(optional.modifyStreamF(Stream::of).apply(s).collect(toList()), Collections.singletonList(s));
  }
  ...
  public IsEq<List<S>> modifyIterableFPoint(S s) {
    return isEq(
      stream(spliteratorUnknownSize(optional.modifyIterableF(Collections::singleton).apply(s).iterator(), ORDERED), false).collect(toList()),
