 ...
 * {@link Optional} is a {@link POptional} restricted to monomorphic updates: {{{ type Optional[S, A] = POptional[S, S, A, A]
 * }}}
 *
...
public abstract class POptional<S, T, A, B> {
  ...
  public static <S, T, A, B> POptional<S, T, A, B> pOptional(final Function<S, Either<T, A>> getOrModify, final Function<B, Function<S, T>> set) {
    return new POptional<S, T, A, B>() {
      ...
      @Override
      public Function<S, Iterable<T>> modifyIterableF(final Function<A, Iterable<B>> f) {
        return s -> getOrModify.apply(s).fold(Collections::singleton, t -> Iterables.<B, T>transform(f.apply(t), b -> set.apply(b).apply(s)));
      }
