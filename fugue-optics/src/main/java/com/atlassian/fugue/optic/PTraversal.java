...
import com.atlassian.fugue.optic.internal.Utils;
 ...
 * {@link Traversal} is a {@link PTraversal} restricted to monomorphic updates.
 *
...
public abstract class PTraversal<S, T, A, B> {
  ...
  public final <S1, T1> PTraversal<Either<S, S1>, Either<T, T1>, A, B> sum(final PTraversal<S1, T1, A, B> other) {
    ...
    return new PTraversal<Either<S, S1>, Either<T, T1>, A, B>() {
      ...
      public Function<Either<S, S1>, Stream<Either<T, T1>>> modifyStreamF(final Function<A, Stream<B>> f) {
        return ss1 -> ss1
          .fold(s -> self.modifyStreamF(f).apply(s).map(Eithers.toLeft()), s1 -> other.modifyStreamF(f).apply(s1).map(Eithers.toRight()));
      }

      @Override
      public Function<Either<S, S1>, Iterable<Either<T, T1>>> modifyIterableF(final Function<A, Iterable<B>> f) {
        return ss1 -> ss1.fold(s -> Iterables.transform(self.modifyIterableF(f).apply(s), Eithers.toLeft()),
          s1 -> Iterables.transform(other.modifyIterableF(f).apply(s1), Eithers.toRight()));
      }
      ...
      public Function<Either<S, S1>, Pair<Either<T, T1>, Either<T, T1>>> modifyPairF(final Function<A, Pair<B, B>> f) {
        return ss1 -> ss1
          .fold(s -> Utils.map(self.modifyPairF(f).apply(s), Eithers.toLeft()), s1 -> Utils.map(other.modifyPairF(f).apply(s1), Eithers.toRight()));
      }

      @Override
      public <M> Function<Either<S, S1>, M> foldMap(final Supplier<M> identity, final BinaryOperator<M> op, final Function<A, M> f) {
  ...
  public final <C, D> PTraversal<S, T, C, D> composeTraversal(final PTraversal<A, B, C, D> other) {
    ...
    return new PTraversal<S, T, C, D>() {
      ...
      @Override
      public <M> Function<S, M> foldMap(final Supplier<M> identity, final BinaryOperator<M> op, final Function<C, M> f) {
  ...
  public final Fold<S, A> asFold() {
    return new Fold<S, A>() {
      @Override
      public <M> Function<S, M> foldMap(final Supplier<M> identity, final BinaryOperator<M> op, final Function<A, M> f) {
  ...
  public static <S, T> PTraversal<Either<S, S>, Either<T, T>, S, T> pCodiagonal() {
    return new PTraversal<Either<S, S>, Either<T, T>, S, T>() {
      ...
      public Function<Either<S, S>, Stream<Either<T, T>>> modifyStreamF(final Function<S, Stream<T>> f) {
        return s -> s.bimap(f, f).fold(stream -> stream.map(Eithers.toLeft()), stream -> stream.map(Eithers.toRight()));
      }

      @Override
      public Function<Either<S, S>, Iterable<Either<T, T>>> modifyIterableF(final Function<S, Iterable<T>> f) {
        return s -> s.bimap(f, f).fold(ts -> Iterables.transform(ts, Eithers.toLeft()), ts -> Iterables.transform(ts, Eithers.toRight()));
      }
      ...
      public Function<Either<S, S>, Pair<Either<T, T>, Either<T, T>>> modifyPairF(final Function<S, Pair<T, T>> f) {
        return s -> s.bimap(f, f).fold(tt -> Utils.map(tt, Eithers.toLeft()), tt -> Utils.map(tt, Eithers.toRight()));
      }

      @Override
      public <M> Function<Either<S, S>, M> foldMap(final Supplier<M> identity, final BinaryOperator<M> op, final Function<S, M> f) {
        return s -> s.fold(f, f);
      }
    };
  }
  ...
  public static <S, T, A, B> PTraversal<S, T, A, B> pTraversal(final Function<S, A> get1, final Function<S, A> get2,
    ...
    return new PTraversal<S, T, A, B>() {
      ...
      public <C> Function<S, Function<C, T>> modifyFunctionF(final Function<A, Function<C, B>> f) {
        return s -> Utils.ap(f.apply(get2.apply(s)), f.apply(get1.apply(s)).andThen(b1 -> b2 -> set.apply(b1, b2).apply(s)));
      }

      @Override
      public <L> Function<S, Either<L, T>> modifyEitherF(final Function<A, Either<L, B>> f) {
        return s -> f.apply(get2.apply(s)).right().apply(f.apply(get1.apply(s)).right().<Function<B, T>>map(b1 -> b2 -> set.apply(b1, b2).apply(s)));
      }
      ...
      public Function<S, Stream<T>> modifyStreamF(final Function<A, Stream<B>> f) {
        return s -> Utils.ap(f.apply(get2.apply(s)), f.apply(get1.apply(s)).<Function<B, T>>map(b1 -> b2 -> set.apply(b1, b2).apply(s)));
      }

      @Override
      public Function<S, Iterable<T>> modifyIterableF(final Function<A, Iterable<B>> f) {
        return s -> Utils.ap(f.apply(get2.apply(s)), Iterables.transform(f.apply(get1.apply(s)), b1 -> b2 -> set.apply(b1, b2).apply(s)));
      }

      @Override
      public Function<S, Supplier<T>> modifySupplierF(final Function<A, Supplier<B>> f) {
        return s -> Utils.ap(f.apply(get2.apply(s)), Suppliers.compose(b1 -> b2 -> set.apply(b1, b2).apply(s), f.apply(get1.apply(s))));
      }

      @Override
      public Function<S, Pair<T, T>> modifyPairF(final Function<A, Pair<B, B>> f) {
        return s -> Utils.ap(f.apply(get2.apply(s)), Utils.map(f.apply(get1.apply(s)), b1 -> b2 -> set.apply(b1, b2).apply(s)));
      }
  ...
  private static <S, T, A, B> PTraversal<S, T, A, B> fromCurried(final PTraversal<S, Function<B, T>, A, B> curriedTraversal,
    ...
    return new PTraversal<S, T, A, B>() {
      ...
      @Override
      public <C> Function<S, Function<C, T>> modifyFunctionF(final Function<A, Function<C, B>> f) {
        return s -> Utils.ap(f.apply(lastGet.apply(s)), curriedTraversal.modifyFunctionF(f).apply(s));
      }

      @Override
      public <L> Function<S, Either<L, T>> modifyEitherF(final Function<A, Either<L, B>> f) {
        return s -> f.apply(lastGet.apply(s)).right().apply(curriedTraversal.modifyEitherF(f).apply(s));
      }
      ...
      @Override
      public Function<S, Stream<T>> modifyStreamF(final Function<A, Stream<B>> f) {
        return s -> Utils.ap(f.apply(lastGet.apply(s)), curriedTraversal.modifyStreamF(f).apply(s));
      }

      @Override
      public Function<S, Iterable<T>> modifyIterableF(final Function<A, Iterable<B>> f) {
        return s -> Utils.ap(f.apply(lastGet.apply(s)), curriedTraversal.modifyIterableF(f).apply(s));
      }

      @Override
      public Function<S, Supplier<T>> modifySupplierF(final Function<A, Supplier<B>> f) {
        return s -> Utils.ap(f.apply(lastGet.apply(s)), curriedTraversal.modifySupplierF(f).apply(s));
      }

      @Override
      public Function<S, Pair<T, T>> modifyPairF(final Function<A, Pair<B, B>> f) {
        return s -> Utils.ap(f.apply(lastGet.apply(s)), curriedTraversal.modifyPairF(f).apply(s));
      }
