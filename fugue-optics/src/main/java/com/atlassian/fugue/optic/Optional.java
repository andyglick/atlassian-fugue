...
public final class Optional<S, A> extends POptional<S, S, A, A> {
  ...
  @Override
  public Function<S, Supplier<S>> modifySupplierF(final Function<A, Supplier<A>> f) {
  ...
  @Override
  public Function<S, Iterable<S>> modifyIterableF(final Function<A, Iterable<A>> f) {
    return pOptional.modifyIterableF(f);
  }

  @Override
  public Function<S, Pair<S, S>> modifyPairF(final Function<A, Pair<A, A>> f) {
  ...
  public static <S, A> Optional<S, A> optional(final Function<S, Option<A>> getOption, final Function<A, Function<S, S>> set) {
    return new Optional<>(new POptional<S, S, A, A>() {
      ...
      @Override
      public Function<S, Iterable<S>> modifyIterableF(final Function<A, Iterable<A>> f) {
        return s -> getOption.apply(s)
          .<Iterable<S>>fold(() -> Collections.singleton(s), t -> Iterables.transform(f.apply(t), b -> set.apply(b).apply(s)));
      }

      @Override
      public Function<S, Supplier<S>> modifySupplierF(final Function<A, Supplier<A>> f) {
        return s -> getOption.apply(s)
          .<Supplier<S>>fold(() -> Suppliers.ofInstance(s), t -> Suppliers.compose(b -> set.apply(b).apply(s), f.apply(t)));
      }
