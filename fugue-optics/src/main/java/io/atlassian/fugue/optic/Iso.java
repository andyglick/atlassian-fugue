...
public final class Iso<S, A> extends PIso<S, S, A, A> {
  ...
  @Override public S reverseGet(final A a) {
    return pIso.reverseGet(a);
  }

  @Override public Iso<A, S> reverse() {
  ...
  @Override public <C> Iso<Pair<S, C>, Pair<A, C>> first() {
    return new Iso<>(pIso.first());
  }

  @Override public <C> Iso<Pair<C, S>, Pair<C, A>> second() {
   ...
   */
  @Override public final Prism<S, A> asPrism() {
   ...
   */
  @Override public final Lens<S, A> asLens() {
    return new Lens<>(pIso.asLens());
  }

  /**
   * create an {@link Iso} using a pair of functions: one to get the target and
