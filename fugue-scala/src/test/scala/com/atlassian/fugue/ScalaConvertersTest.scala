...
class ScalaConvertersTest {
  ...
  @Test def fromSupplierLazily() {
    lazy val converted: () => String = Suppliers.fromOption[String](Option.none()).toScala
  ...
  @Test def function1ToGuavaFunctionImplicitly() {
    val g: GuavaFunction[String, Integer] = { (s: String) => s.length }.toJava
    assertEquals(3, g.apply("abc"))
  }

  @Test def function1ToGuavaFunctionTypeConvertedExplicitly() {
    val g: GuavaFunction[Integer, Integer] = ((i: Int) => i * 2).toJava
  ...
  @Test def function2ToFugueFunction2Implicitly() {
    val f: Function2[String, Integer, String] = { (s: String, i: Int) => s * i }.toJava
  ...
  @Test def optionToFugueOption() {
    val o: Option[Integer] = Some(1).asInstanceOf[scala.Option[Int]].toJava
    assertEquals(Option.some(1), o)
  }

  @Test def fugueOptionToOption() {
    val o: scala.Option[Int] = Option.some(Integer.valueOf(1)).toScala
    assertEquals(Some(1), o)
  }

  @Test def eitherToFugueEitherTypeConvertedExplicitly() {
    val e: Either[String, Integer] = Right(1).asInstanceOf[scala.Either[String, Int]].toJava
    assertEquals(Either.right[String, Integer](1), e)
  }

  @Test def tuple2ToFuguePairImplicitly() {
    val p: Pair[String, Integer] = ("abc", 1).toJava
    assertEquals("abc", p.left())
    assertEquals(1, p.right())
  }

  @Test def fuguePairToTuple2() {
    val p: (String, Int) = Pair.pair("abc", Integer.valueOf(1)).toScala
