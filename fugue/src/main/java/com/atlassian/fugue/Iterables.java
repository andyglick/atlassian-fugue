 ...
 *
 * @since 3.0
 * @deprecated in 3.0 moved to com.atlassian.fugue.collect.Iterables
 */
@Deprecated
public class Iterables {
  ...
  static final Iterable<?> EMPTY = new Iterable<Object>() {
    @Override public Iterator<Object> iterator() {
      return DeprecatedIterators.emptyIterator();
    }
  ...
  public static Iterable<Integer> rangeTo(final int start, final int end, final int step) {
    ...
    return () -> new DeprecatedUnmodifiableIterator<Integer>() {
      private int i = start;
  ...
  static final class Range<A> extends IterableToString<A> {
    ...
    static final class Iter<T> extends DeprecatedAbstractIterator<T> {
  ...
  static class CollectingIterable<A, B> extends IterableToString<B> {
    ...
    final class Iter extends DeprecatedAbstractIterator<B> {
  ...
  static final class Intersperse<A> implements Iterable<A> {
    ...
    @Override public Iterator<A> iterator() {
      return new DeprecatedAbstractIterator<A>() {
  ...
  static final class Transform<A, B> implements Iterable<B> {
    ...
    @Override public Iterator<B> iterator() {
      return new DeprecatedAbstractIterator<B>() {
        private final Iterator<? extends A> it = as.iterator();
  ...
  static final class Flatten<A> implements Iterable<A> {
    ...
    @Override public Iterator<A> iterator() {
      return new DeprecatedAbstractIterator<A>() {
        ...
        private Iterator<? extends A> currentIterator = DeprecatedIterators.emptyIterator();
  ...
  public static <A> boolean addAll(Collection<A> collectionToModify, Iterable<? extends A> elementsToAdd) {
    ...
    return DeprecatedIterators.addAll(collectionToModify, requireNonNull(elementsToAdd).iterator());
  }
  ...
  static final class MergeSortedIterable<A> extends IterableToString<A> {
    ...
    private static final class Iter<A> extends DeprecatedAbstractIterator<A> {
      private final TreeSet<DeprecatedPeekingIterator<A>> xss;

      private Iter(final Iterable<? extends Iterable<A>> xss, final Comparator<A> c) {
        this.xss = new TreeSet<>(DeprecatedPeekingIteratorComparator(c));
        addAll(this.xss, transform(filter(xss, isEmpty().negate()), i -> DeprecatedIterators.DeprecatedPeekingIterator(i.iterator())));
      }

      @Override protected A computeNext() {
        final Option<DeprecatedPeekingIterator<A>> currFirstOption = first(xss);
        if (!currFirstOption.isDefined()) {
          return endOfData();
        }
        final DeprecatedPeekingIterator<A> currFirst = currFirstOption.get();
      ...
      private Comparator<? super DeprecatedPeekingIterator<A>> DeprecatedPeekingIteratorComparator(final Comparator<A> comparator) {
  ...
  static final class Memoizer<A> extends IterableToString<A> {
     ...
     */
    static class Lazy<A> extends DeprecatedLazyReference<Node<A>> implements Node<A> {
    ...
    static class Iter<A> extends DeprecatedAbstractIterator<A> {
      Node<A> node;
