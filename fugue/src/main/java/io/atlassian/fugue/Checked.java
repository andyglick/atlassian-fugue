...
public class Checked {
  ...
  /**
   * Lifts a function that potentially throws into a function that either returns a delayed Try, which, when evaluated,
   * returns a Success of the result or a failure containing the thrown exception.
   *
   * @param f a function that can throw
   * @param <A> the function argument type
   * @param <B> the function return type
   ...
   * @deprecated since 4.6 This is being replaced with
   * {@link #now(Supplier)} to make it clear that the supplier is evaluated
   * immediately. If the evaluation needs to be delayed, use {@link #delay(Supplier)}.
   */
  @Deprecated
  ...
  /**
   * Create a new Try representing the result of a potentially exception
   * throwing operation. The provided supplier is evaluated immediately.
  ...
  /**
   * Create a new delayed Try representing a delayed evaluation of a potentially exception
   * throwing operation. The provided supplier is only called when the delayed Try is evaluated.
   * The provided supplier is called only once no matter how many times the returned delayed Try is evaluated.
   * If the provided supplier throws an exception the evaluation of the delayed Try will
   * return a failure wrapping the exception, otherwise a success of the supplied value will be returned.
   *
   * @param s a supplier that may throw an exception
   * @param <A> the type of value s supplies
   * @param <E> The type of exception potentially thrown
   * @return a delayed Try. If s throws an exception this, when evaluated, will return a failure wrapping the
   * exception, otherwise a success of the supplied value.
   */
