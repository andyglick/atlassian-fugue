...
public final class MonoidLaws<A> {
   ...
   *
   * @param monoid a {@link io.atlassian.fugue.Monoid} to check matches the desired behaviors of
   */
  ...
  /**
   * A monoid must not care about the order elements are combined. If you combine x with y and then with z the result should be the same as combining
   * y with z and then with x.
   *
   * @param x an A object
   * @param y an A object
   * @param z an A object
   * @return a {@link io.atlassian.fugue.law.IsEq} instance where append(append(x,y),z) is equal to append(x, append(y,z))
   */
   ...
   * @return a {@link io.atlassian.fugue.law.IsEq} where x is equal to append(zero(), x)
   */
   ...
   *
   * @param x an element of your monoid type
   * @return a {@link io.atlassian.fugue.law.IsEq} where x is equal to append(x, zero())
   */
  ...
  /**
   * The sum function of your monoid must be equal to {@link Functions#fold(BiFunction, Object, Iterable)} using append as the folding function and zero() as the initial value
   *
   * @param as a {@link java.lang.Iterable}
   * @return a {@link io.atlassian.fugue.law.IsEq} where sum(as) is equal to fold(append, zero, as)
   */
  ...
  /**
   * The multiply function of your monoid must be equal to the sum function called with an iterable containing {@code n} copies of the input type.
   *
   * @param n a int
   * @param a an A
   * @return a {@link io.atlassian.fugue.law.IsEq} where multiply(n,a) is equal to sum(take(n, cycle(a))
   */
