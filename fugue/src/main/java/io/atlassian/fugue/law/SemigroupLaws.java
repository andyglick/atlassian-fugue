...
public final class SemigroupLaws<A> {
   ...
   *
   * @param semigroup a {@link io.atlassian.fugue.Semigroup} to check matches the desired behaviors of
   */
  ...
  /**
   * A semigroup must not care about the order elements are combined. If you combine x with y and then with z the result should be the same as combining
  ...
  /**
   * The {@link Semigroup#sumNonEmpty(Object, Iterable)} function of your semigroup must be equal to a {@link Functions#fold(BiFunction, Object, Iterable)}
   * where append is used as the combining function, head is used as the initial value and tail is the iterable to check
   *
   * @param head an A
   * @param tail a {@link java.lang.Iterable} of A's
   * @return a {@link io.atlassian.fugue.law.IsEq} where sumeNonEmpty(head, tail) is equal to fold(append, head, tail)
   */
  ...
  /**
   * The {@link Semigroup#multiply1p(int, Object)} function of your semigroup must be equal to {@link Semigroup#sumNonEmpty(Object, Iterable)} applied to
   ...
   * @param a an A
   * @return a {@link io.atlassian.fugue.law.IsEq} where multiply(n,a) is equal to sumNonEmpty(a, take(n-1, cycle(a))
   */
