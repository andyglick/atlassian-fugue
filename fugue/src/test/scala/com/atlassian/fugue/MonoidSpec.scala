...
class MonoidSpec extends TestSuite {
  ...
  test("Monoid laws") {check(MonoidTests(intMonoid))}

  test("Monoid derived functions") { check(MonoidDerivedFunctionsTests(intMonoid))}

  val mulMonoid = monoid(semigroup((i1: Int, i2: Int) => i1 * i2), 1)

  test("Monoid composition") { check(MonoidTests(intMonoid.composeMonoid(mulMonoid))) }
