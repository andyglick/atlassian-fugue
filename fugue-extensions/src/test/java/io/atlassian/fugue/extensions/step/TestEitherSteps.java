...
public class TestEitherSteps {
  ...
  private static final Supplier<AnError> filterAnErrorFunction = () -> AnError.FILTER;
  ...
  @Test public void test_1_step_filter_success() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).filter(alwaysTrue(), filterAnErrorFunction).yield(Long::new);

    assertThat(stepped, isRight(is(LONG)));
  }

  @Test public void test_1_step_filter_failure() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).filter(alwaysFalse(), filterAnErrorFunction).yield(Long::new);
  ...
  @Test public void test_2_step_filter_success() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).filter(alwaysTrue2(), filterAnErrorFunction).yield((v1, v2) -> v2);

    assertThat(stepped, isRight(is(LONG)));
  }

  @Test public void test_2_step_filter_failure() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).filter(alwaysFalse2(), filterAnErrorFunction).yield((v1, v2) -> v2);

    assertThat(stepped, isLeft(is(AnError.FILTER)));
  }

  @Test public void test_2_step_filter_failure_1() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).filter(alwaysFalse(), filterAnErrorFunction).then(() -> ok(LONG)).yield((v1, v2) -> v2);
  ...
  @Test public void test_3_step_filter_success() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).then(() -> ok(STRING_LOWERED))
      .filter(alwaysTrue3(), filterAnErrorFunction).yield((v1, v2, v3) -> v2);
  ...
  @Test public void test_3_step_filter_failure() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).then(() -> ok(STRING_LOWERED))
      .filter(alwaysFalse3(), filterAnErrorFunction).yield((v1, v2, v3) -> v2);

    assertThat(stepped, isLeft(is(AnError.FILTER)));
  }

  @Test public void test_3_step_filter_failure_1() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).filter(alwaysFalse(), filterAnErrorFunction).then(() -> ok(LONG))
  ...
  @Test public void test_3_step_filter_failure_2() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).filter(alwaysFalse2(), filterAnErrorFunction)
      .then(() -> ok(STRING_LOWERED)).yield((v1, v2, v3) -> v2);
  ...
  @Test public void test_4_step_filter_success() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).then(() -> ok(STRING_LOWERED)).then(() -> ok(true))
      .filter(alwaysTrue4(), filterAnErrorFunction).yield((v1, v2, v3, v4) -> v2);
  ...
  @Test public void test_4_step_filter_failure() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).then(() -> ok(STRING_LOWERED)).then(() -> ok(true))
      .filter(alwaysFalse4(), filterAnErrorFunction).yield((v1, v2, v3, v4) -> v2);
  ...
  @Test public void test_4_step_filter_failure_3() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(() -> ok(LONG)).then(() -> ok(STRING_LOWERED))
      .filter(alwaysFalse3(), filterAnErrorFunction).then(() -> ok(true)).yield((v1, v2, v3, v4) -> v2);
  ...
  @Test public void test_5_step_filter_success() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(v -> ok(STRING)).then((first, second) -> ok(first + second))
      .then((v1, v2, v3) -> ok(STRING)).then((v1, v2, v3, v4) -> ok(STRING)).filter(alwaysTrue5(), filterAnErrorFunction)
      .yield((value1, value2, value3, value4, value5) -> new Long(value3));
  ...
  @Test public void test_5_step_filter_failure() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(v -> ok(STRING)).then((first, second) -> ok(first + second))
      .then((v1, v2, v3) -> ok(STRING)).then((v1, v2, v3, v4) -> ok(STRING)).filter(alwaysFalse5(), filterAnErrorFunction)
      .yield((value1, value2, value3, value4, value5) -> new Long(value3));
  ...
  @Test public void test_5_step_filter_failure_1() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).filter(alwaysFalse(), filterAnErrorFunction).then(v -> ok(STRING))
  ...
  @Test public void test_5_step_filter_failure_2() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(v -> ok(STRING)).filter(alwaysFalse2(), filterAnErrorFunction)
      .then((first, second) -> ok(first + second)).then((v1, v2, v3) -> ok(STRING)).then((v1, v2, v3, v4) -> ok(STRING))
      .yield((value1, value2, value3, value4, value5) -> new Long(value3));
  ...
  @Test public void test_5_step_filter_failure_3() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(v -> ok(STRING)).then((first, second) -> ok(first + second))
      .filter(alwaysFalse3(), filterAnErrorFunction).then((v1, v2, v3) -> ok(STRING)).then((v1, v2, v3, v4) -> ok(STRING))
      .yield((value1, value2, value3, value4, value5) -> new Long(value3));
  ...
  @Test public void test_5_step_filter_failure_4() {
    Either<AnError, Long> stepped = Steps.begin(ok(STRING)).then(v -> ok(STRING)).then((first, second) -> ok(first + second))
      .then((v1, v2, v3) -> ok(STRING)).filter(alwaysFalse4(), filterAnErrorFunction).then((v1, v2, v3, v4) -> ok(STRING))
      .yield((value1, value2, value3, value4, value5) -> new Long(value3));
  ...
  @Test public void test_6_step_filter_success() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).then(() -> ok(STRING)).then(() -> ok(STRING + STRING)).then(() -> ok(STRING))
      .then(() -> ok(STRING_UPPERED)).then(() -> ok(STRING_LOWERED)).filter(alwaysTrue6(), filterAnErrorFunction)
  ...
  @Test public void test_6_step_filter_failure() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).then(() -> ok(STRING)).then(() -> ok(STRING + STRING)).then(() -> ok(STRING))
      .then(() -> ok(STRING_UPPERED)).then(() -> ok(STRING_LOWERED)).filter(alwaysFalse6(), filterAnErrorFunction)
      .yield((value1, value2, value3, value4, value5, value6) -> value6);
  ...
  @Test public void test_6_step_filter_failure_1() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).filter(alwaysFalse(), filterAnErrorFunction).then(() -> ok(STRING))
  ...
  @Test public void test_6_step_filter_failure_2() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).then(() -> ok(STRING)).filter(alwaysFalse2(), filterAnErrorFunction)
      .then(() -> ok(STRING + STRING)).then(() -> ok(STRING)).then(() -> ok(STRING_UPPERED)).then(() -> ok(STRING_LOWERED))
      .yield((value1, value2, value3, value4, value5, value6) -> value6);
  ...
  @Test public void test_6_step_filter_failure_3() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).then(() -> ok(STRING)).then(() -> ok(STRING + STRING))
      .filter(alwaysFalse3(), filterAnErrorFunction).then(() -> ok(STRING)).then(() -> ok(STRING_UPPERED)).then(() -> ok(STRING_LOWERED))
      .yield((value1, value2, value3, value4, value5, value6) -> value6);
  ...
  @Test public void test_6_step_filter_failure_4() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).then(() -> ok(STRING)).then(() -> ok(STRING + STRING)).then(() -> ok(STRING))
      .filter(alwaysFalse4(), filterAnErrorFunction).then(() -> ok(STRING_UPPERED)).then(() -> ok(STRING_LOWERED))
      .yield((value1, value2, value3, value4, value5, value6) -> value6);
  ...
  @Test public void test_6_step_filter_failure_5() {
    Either<AnError, String> stepped = Steps.begin(ok(STRING)).then(() -> ok(STRING)).then(() -> ok(STRING + STRING)).then(() -> ok(STRING))
      .then(() -> ok(STRING_UPPERED)).filter(alwaysFalse5(), filterAnErrorFunction).then(() -> ok(STRING_LOWERED))
      .yield((value1, value2, value3, value4, value5, value6) -> value6);
